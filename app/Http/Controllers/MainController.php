...
class MainController extends BaseController
{
    ...
    public function homePage()
    {
        ...
        //Move all error file generated by dusk to the latest test directory.
        $error_files = array_filter(
            scandir(public_path('screenshot/')), function ($item) {
                return !is_dir(public_path('screenshot/') . $item);
            }
        );
        $target_dir=public_path('screenshot/'.end($a_dir).'/SystemError/');
        if (!file_exists($target_dir)) {
            mkdir($target_dir, 0777, true);
        }
        foreach($error_files as $error_file)
        {
             rename(public_path('screenshot/').$error_file, $target_dir.$error_file);
        }
